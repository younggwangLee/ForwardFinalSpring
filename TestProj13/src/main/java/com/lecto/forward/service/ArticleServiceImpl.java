...
public class ArticleServiceImpl implements ArticleService{
	...
	public List<ArticleVO> searchArticle(String boardCode, String searchWay, String keyword, Criteria cri){
		...
		if(searchWay.equals("nickname")) {
			searchWayCode = "1";
		}else if(searchWay.equals("title")) {
			searchWayCode = "2";
		...
		try {
			...
			list = articleViewMapper.searchArticleKeyword(boardCode, searchWayCode, keyword, cri);
		} catch (Exception e) {
	...
	/* 해당 게시판에 있는 모든 게시글 가져오기 (페이징) */
	public List<ArticleVO> searchArticle(String boardCode, Criteria cri) {
		List<ArticleVO> articles = null;
		try {
			articles = articleViewMapper.searchArticle(boardCode, cri);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public List<ArticleVO> searchArticle(String boardCode) {
		List<ArticleVO> articles = null;
		try {
			articles = articleViewMapper.searchArticles(boardCode);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	@Override
	public int listCountCriteria(String boardCode) throws Exception {
		return articleViewMapper.countPaging(boardCode);
	}
