...
public class MemberServiceImpl implements MemberService{
			...
			for(BoardDTO dto: blist){
				if(dto.getBoardName().equals(boardName)){
					try {
						...
						memberGradesMapper.addMemberGradesDTO(mg);
						flag = true;
						if(memberGradesMapper.searchBoardGrades(dto.getBoardCode(), "admin") != null) {
							memberGradesMapper.deleteDefaultBoard(dto.getBoardCode(), "admin");
						}
					} catch(Exception e) {
						e.printStackTrace();
						flag = false;
	...
	public Object[] searchManager(Criteria cri) {
		...
		try {
			list = managerViewMapper.showManagers(cri);
		} catch (Exception e) {
			e.printStackTrace();
		}
		int j = 0;
		for(int i=0; i<list.size(); i++){
			if(!list.get(i).getMemberId().equals("admin")) {
				j++;
	...
	@Override
	public int listCountCriteria(String boardName) {
		return bmlViewMapper.countPaging(boardName);
	}
	
	@Override
	public int managerListCountCriteria() throws Exception {
		return managerViewMapper.countPaging();
	}
}