...
<link rel='stylesheet' id='jquery-ui-css'  href='http://192.168.43.115:8083/webProject/jquery-ui.min.css' type='text/css' media='all' />
<link rel='stylesheet' id='unslider-css'  href='http://192.168.43.115:8083/webProject/unslider.css' type='text/css' media='all' />
<!--[if lt IE 9]>
...
<link rel="alternate" type="text/xml+oembed" href="http://192.168.43.115:8083/webProject/json/embed.json" />
...
<div id="masthead" class="site-header" role="banner"><!-- site-header-menu 영역 호버시 nav_on 클래스 적용   .search_btn 클릭시 search_on 클래스 적용-->
			...
			<form action="/m_signuppage" method="GET" id="signupPage"></form>
			<!-- 데이터 -->
			<!-- 서치박스 -->
				...
				<div class="max_container">
					...
					<div class="head_linkbx">
						<ul>
							...
							<li><a href="/m_signuppage" id="join">JOIN</a></li>
							</c:if>
						...
						<nav id="site-navigation" class="main-navigation" role="navigation" aria-label="기본 메뉴">
							...
							<ul class="sub-menu">
								<c:forEach var="boardDTO" items="${boardList}">
									<li class="menu-item menu-item-type-post_type menu-item-object-page menu-item-92 board" value="${boardDTO.boardName}"><a href="m_board?boardCode=${boardDTO.boardCode}&page=1">${boardDTO.boardName}</a></li>
								</c:forEach> 
									<ul class="sub-menu">
...
$(document).ready(function(){
	...
	$(".sub-menu li").on("click", function(e){
		$form = '<form></form>'
		$form.append('<input type="hindden" id="boardList" value="${boardList}">');
		$form.append('<input type="hindden" id="boardName" value="'$(this).val()+'">');
		$form.append('<input type="hindden" id="page" value="${boardList}">');
		$form.attr('action', '');
