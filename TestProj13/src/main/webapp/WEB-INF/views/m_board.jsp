...
<div id="primary" class="content-area">
	<div id="main" class="site-main" role="main">
		<div class="content_rowbx">
			<div class="max_container" id="max_container22">
				...
				<c:if test="${themeCode eq 'th2'}">
						<c:if test="${boardDTO.boardName eq '자유게시판'}">
								<!-- 배경에 축구 이미지가 나온다. -->
								<link rel="stylesheet" type="text/css" href="http://192.168.43.115:8083/webProject/css/m_board_freeboard_background.css">
						</c:if>
						
						<c:if test="${boardDTO.boardName eq '공지사항'}">
								<!-- 배경에 공지게시판 모양가 나온다. -->
								<link rel="stylesheet" type="text/css" href="http://192.168.43.115:8083/webProject/css/m_board_notice_background.css">
						</c:if>
						
						<c:if test="${boardDTO.boardName eq '베드민턴'}">
								<!-- 배경에 베드민턴 이미지가 나온다. -->
								<link rel="stylesheet" type="text/css" href="http://192.168.43.115:8083/webProject/css/m_board_badmintan_background.css">
						</c:if>
						
						<c:if test="${boardDTO.boardName eq '야구게시판'}">
								<!-- 배경에 축구 이미지가 나온다. -->
								<link rel="stylesheet" type="text/css" href="http://192.168.43.115:8083/webProject/css/m_board_baseball_background.css">
						</c:if>
						
						<c:if test="${boardDTO.boardName eq '축구게시판'}">
								<!-- 배경에 축구 이미지가 나온다. -->
								<link rel="stylesheet" type="text/css" href="http://192.168.43.115:8083/webProject/css/m_board_soccer_background.css">
						</c:if>
						
						<c:if test="${boardDTO.boardName eq '농구게시판'}">
								<!-- 배경에 축구 이미지가 나온다. -->
								<link rel="stylesheet" type="text/css" href="http://192.168.43.115:8083/webProject/css/m_board_basketball_background.css">
						</c:if>
								...
								<!-- <link rel="stylesheet" type="text/css" href="./resources/css/theme2.css"> -->
									<table border="1" id="tempTable">
										<tr>
											<td id="td1">
											<div id="actionImage"> </div></td>
	
											<td id="td2">
											<div class="selectbx">
											<select style="width: 70px;" name="selectWay" id="selectBox">
												<option value="title">제목</option>
												<option value="content">내용</option>
												<option value="nickname">작성자</option>
											</select>
											...
											<td id="td3"><a href="/a_articlelist/search?selectBoardName=${boradDTO.boardName}" class="btns" id="searchButton">검색</a></td>
											
											<td id="td4"><c:if test ="${sessionScope.login != null}">	
										...
										<tbody id="articleList">
											<c:forEach items="${list}" var="articleVO" varStatus="status">
												<tr>
													...
													<td>${articleVO.articleDate}</td>
													<!-- 조회수-->
													<td><span class="badge bg-red"  id="hitsShape">${articleVO.articleHits}</span></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
...
<script src="http://192.168.43.115:8083/webProject/js/jquery-3.3.1.min.js"></script>
<script>
